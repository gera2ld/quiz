...
class API {
  ...
  getList(path, ...args) {
    const middlewares = this.buildMiddlewares(args, options => {
      ...
      return wraps(function* (next) {
        const ctx = this;
        const list = yield Model.findAll(getQuery(ctx, query));
        ctx.body = {
  ...
  get(path, ...args) {
    const middlewares = this.buildMiddlewares(args, options => {
      ...
      return wraps(function* (next) {
        ...
        const item = yield Model.findById(id, getQuery(ctx, query));
  ...
  _patch(method, path, ...args) {
    const middlewares = this.buildMiddlewares(args, options => {
      ...
      return wraps(function* (next) {
        ...
        const item = yield Model.findById(id, getQuery(ctx, query));
        if (!item) return setError(ctx, 404, 'Not found');
        yield item.update(updates);
        if (after) yield* after(body, ctx, item);
        ctx.body = {
          data: item.toJSON(),
        };
  ...
  post(path, ...args) {
    const middlewares = this.buildMiddlewares(args, options => {
      ...
      return wraps(function* (next) {
        ...
        const item = yield Model.create(data, getQuery(ctx, query));
        if (after) yield* after(body, ctx, item);
        ctx.status = 201;
        ctx.body = {
          data: item.toJSON(),
  ...
  buildDocs() {
    const {docs} = this;
    this.router
    .get('/_docs/', function* (next) {
      this.set('Content-Type', 'text/html; charset=utf-8');
      this.body = require('fs').createReadStream(__dirname + '/../static/index.html', 'utf8');
    })
    .get('/_docs/items', function* (next) {
      this.body = {
        data: docs,
      };
    });
